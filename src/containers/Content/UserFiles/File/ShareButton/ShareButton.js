  ...
  submitHandler = async (e) => {
    ...
    try {
      let userExists = await Axios.get(
        "http://localhost:3001/user/acquire/key/" + this.state.value
      );

      let contractUpdateData = await this.state.contract.methods
        .shareFile(
          this.props.values.file.hash,
          this.context.userPublicKey,
          userExists.data.key
        )
      ...
      if (contractUpdateData.events["FileShared"]) {
        ...
        let fileCloudShare = await Axios.post(
          "http://localhost:3001/cloud/upload/dup",
          {
            hash: this.props.values.file.hash,
            owner: userExists.data.key,
