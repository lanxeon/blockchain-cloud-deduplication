    ...
    try {
      ...
      if (this.state.userPublicKey && this.state.FileBlockchain) {
        let userExists = await axios.get(
          "http://localhost:3001/user/" + this.state.userPublicKey
        );
        if (!userExists.data) {
  ...
  fileUploadHandler = async (file) => {
    ...
    fr.onload = async () => {
      try {
        ...
        this.updateProgress(
          0,
          "Waiting for user to accept/reject the transaction",
          70
        );
        ...
        if (results.events["NewUpload"]) {
          ...
          let uploaded = await axios.post(
            "http://localhost:3001/cloud/upload/new",
            formData
          );
        ...
        //Duplicate file, register owner as user(if file doesn't exist, then reupload the file as well)
          ...
          let fileExists = await axios.get(
            "http://localhost:3001/cloud/integrity/" + hash
          );
          ...
          if (fileExists.data) {
            ...
            formData.append(
              "hash",
              results.events["DuplicateUpload"].returnValues.fh
            );
            formData.append(
              "owner",
              results.events["DuplicateUpload"].returnValues.addr
            );
        ...
        //duplicate file and user, need to verify if file exists just in case, if it doesn't, then reupload the file as well
          ...
          if (fileExists.data) {
            ...
            formData.append(
              "hash",
              results.events["DuplicateUploadAndUser"].returnValues.fh
            );
            formData.append(
              "owner",
              results.events["DuplicateUploadAndUser"].returnValues.addr
            );
            formData.append("name", file.name);
            formData.append("fileSize", file.size);
            formData.append("file", file, file.name);
    ...
    let Page = (
      <React.Fragment>
        {!this.state.userExists ? (
          <CreateUser userPublicKey={this.state.userPublicKey} />
        ) : null}
        ...
        {this.state.ongoingUploads.length > 0 ? (
          <Progress files={this.state.ongoingUploads} />
        ) : null}
      </React.Fragment>
    );
